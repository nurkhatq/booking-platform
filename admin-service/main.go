...
import (
    ...
    "booking-platform/admin-service/services"
...
func main() {
    ...
    // Start gRPC server
    go func() {
        lis, err := net.Listen("tcp", fmt.Sprintf(":%d", cfg.Services.AdminGRPC))
        if err != nil {
        ...
        log.Printf("Admin Service gRPC server listening on port %d", cfg.Services.AdminGRPC)
    ...
    go func() {
        port := fmt.Sprintf(":%d", cfg.Services.AdminHTTP)
        log.Printf("Admin Service HTTP server listening on port %s", port)
