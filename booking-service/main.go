...
import (
    ...
    "booking-platform/booking-service/services"
...
func main() {
    ...
    // Start gRPC server
    go func() {
        lis, err := net.Listen("tcp", fmt.Sprintf(":%d", cfg.Services.BookingGRPC))
        if err != nil {
        ...
        log.Printf("Booking Service gRPC server listening on port %d", cfg.Services.BookingGRPC)
    ...
    go func() {
        port := fmt.Sprintf(":%d", cfg.Services.BookingHTTP)
        log.Printf("Booking Service HTTP server listening on port %s", port)
