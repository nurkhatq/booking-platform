...
import (
    ...
    "booking-platform/payment-service/services"
...
func main() {
    ...
    // Start gRPC server
    go func() {
        lis, err := net.Listen("tcp", fmt.Sprintf(":%d", cfg.Services.PaymentGRPC))
        if err != nil {
        ...
        log.Printf("Payment Service gRPC server listening on port %d", cfg.Services.PaymentGRPC)
    ...
    go func() {
        port := fmt.Sprintf(":%d", cfg.Services.PaymentHTTP)
        log.Printf("Payment Service HTTP server listening on port %s", port)
