...
func main() {
    ...
    // Start gRPC server
    go func() {
        lis, err := net.Listen("tcp", fmt.Sprintf(":%d", cfg.Services.UserGRPC))
        if err != nil {
        ...
        log.Printf("User Service gRPC server listening on port %d", cfg.Services.UserGRPC)
    ...
    go func() {
        port := fmt.Sprintf(":%d", cfg.Services.UserHTTP)
        log.Printf("User Service HTTP server listening on port %s", port)
